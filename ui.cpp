...
void run(cpu& systemCpu){
    std::cout << "Run" << std::endl;
    for (int i = 0; i <= assemblyCounter; i++){
        systemCpu.tick();
    }
}
...
int main(){
    ...
    std::string labelMemory = "Show Memory";

    Component button_step = Button(&labelStep, [&] { step(systemCpu);}); 
    Component button_run = Button(&labelRun, [&] { run(systemCpu);});
    //Component button_terminal = Button(&labelRun, [&] { run(systemCpu);});	
    Component button_memory = Button(&labelMemory, [&] { openMemory();});	
    //Component button_peripherals = Button(&labelRun, [&] { run(systemCpu);});	
    ...
    auto terminal_view = Renderer(components, [&] {
    return hbox({
            // Left Column
            hbox({
                vbox({
    ...
    auto memory_view = Renderer([&] {
            Elements lines;
            lines.push_back(text("Memory Hex Dump (First 256 Bytes)") | bold | color(Color::Green));
            lines.push_back(separator());
            
            // Render memory in rows of 16 bytes
            for (int row = 0; row < 16; row++) {
                Elements row_elems;
                stringstream addr_ss;
                addr_ss << "0x" << setfill('0') << setw(4) << hex << (row * 16) << ": ";
                row_elems.push_back(text(addr_ss.str()) | color(Color::Yellow));
                
                for (int col = 0; col < 16; col++) {
                    stringstream val_ss;
                    val_ss << setfill('0') << setw(2) << hex << (int)onchipRam[row * 16 + col] << " ";
                    row_elems.push_back(text(val_ss.str()));
                }
                lines.push_back(hbox(row_elems));
            }
            return vbox(lines) | border | vscroll_indicator | frame | flex;
        });

        auto peripheral_view = Renderer([&] {
            return vbox({ text("Peripheral status: OK") | color(Color::Green) }) | border | flex;
        });
    ...
    // 3. Assemble final component tree
    auto main_container = Container::Vertical({
        tab_toggle,  // Tab switching UI at the top
        tab_content, // The active page
    });

    // Handle inputs for both the toggle and the active tab
    auto final_renderer = Renderer(main_container, [&] {
        return vbox({
            text("RISC-V ALPHA EMULATOR") | center | bold,
